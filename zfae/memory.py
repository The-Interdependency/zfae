...
Ported from The-Interdependency/a0 python/engine/memory_core.py.
"""
...
class MemoryCore:
    ...
    def __init__(self, n: int, seed: int, role: str, phases: int = PHASES):
        self.n = n
        self.seed = seed
        self.role = role
        self.phases = phases
    ...
    def _recompute_hub_avg(self) -> None:
        for i in range(self.n):
            self.hub_avg[i] = float(self.tensor[i, :, :, 6].mean())

    def write(self, signal: np.ndarray, alpha: float = 0.30) -> None:
    ...
    def absorb(self, other_tensor: np.ndarray, alpha: float = FLUSH_ALPHA) -> None:
    ...
    def _reset(self) -> None:
        ...
        self.tensor = rng.uniform(0.1, 0.5, (self.n, DIMS, self.phases, HEPT_SITES)).astype(
            np.float64
        )
        self._recompute_hub_avg()
