...
Ported from The-Interdependency/a0 python/engine/ptca_core.py.
DB / sigma / storage deps removed for standalone operation.
...
# Use interdependent-lib's Tensor as a thin wrapper where useful
try:
    from interdependent_lib.ptca.tensor import Tensor as _LibTensor  # noqa: F401
    _HAS_LIB_TENSOR = True
except Exception:
    _HAS_LIB_TENSOR = False
...
class PTCACore:
    ...
    def __init__(
        self, name: str, symbol: str, role: str,
        n: int, seed: int, phases: int = PHASES,
    ):
        self.name = name
        self.symbol = symbol
        self.role = role
    ...
    def _propagate_node(self, i: int) -> None:
        ...
        if signal.ndim == 1 and signal.shape[0] == self.n:
            ...
            for i in range(self.n):
                self.tensor[i, :, 0, :] = np.clip(
                    self.tensor[i, :, 0, :] * 0.85 + signal[i, :, np.newaxis] * 0.15,
                    0.0, 1.0,
                )
